                        ...
                        <div class="main_property_price pt-4 text-muted">
                            ...
                            @else
                                @if($property->sale == true && !empty($property->sale_price) && $property->rent == true && !empty($property->rent_price))
                                    <p class="main_properties_price">
                                        Valor do Imóvel: R$ {{ $property->sale_price }}
                                        <br> ou
                                        Valor do Aluguel: R$ {{ $property->rent_price }}/mês
                                    </p>
                        ...
                        <div class="main_property_contact">
                            ...
                            <form action="{{ route('web.sendEmail') }}" method="POST" autocomplete="off">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="message" name="message" cols="30" rows="5" class="form-control">Quero ter mais informações sobre esse imóvel. Imóvel Residencial, Casa, Campeche, Florianópolis! (#01)</textarea>
                                </div>
...
@section('js')
    <script>
        ...
        function markMap() {
            ...
            var locationJson = $.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address={{ $property->street }},+{{ $property->number }}+{{ $property->city }}+{{ $property->neighborhood }}&key=AIzaSyBAgNpXEFvDrln5Un9iTeGxGG_HFRlZGHA', function(response){
